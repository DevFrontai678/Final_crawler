...
function detectWorkwiseUrl(company) {
    ...
    if (candidate.length > 3) {
        const patterns = [
            `https://${candidate}.workwise.io`,
            `https://${candidate}.workwise.de`,
            `https://workwise.io/company/${candidate}`,
            `https://workwise.de/company/${candidate}`
        ];
        baseUrl = patterns[0];
        slug = candidate;
        console.log(`   ⚠️ Guessing Workwise base URL: ${baseUrl}`);
        return { baseUrl, slug };
    }
...
// ─── PROCESS ONE COMPANY ──────────────────────────────────────────────
async function processWorkwiseCompany(company) {
    console.log(`\n🔍 Processing: ${company.Name}`);
    console.log(`   URL: ${company.detected_career_url}`);

    const wwInfo = detectWorkwiseUrl(company);
    if (!wwInfo) {
        console.log(`   ❌ Could not detect Workwise URL for ${company.Name}`);
        return { company, jobs: [], error: 'No URL detected' };
    }

    const { baseUrl, slug } = wwInfo;
    ...
    const apiEndpoints = [
        ...
        `${baseUrl}/v1/jobs`,
        `${baseUrl}/v1/jobs.json`,
        `${baseUrl}/xml/jobs`,
        `${baseUrl}/api/jobs/feed`
    ];
    ...
    // ─── LAYER 2: HTML Scraping on the Workwise base URL ────────────
    if (jobs.length === 0) {
        console.log(`   🔄 Layer 2: HTML scraping (Workwise base URL)...`);
        try {
            const agent = new https.Agent({ rejectUnauthorized: false });
...
async function run() {
    ...
    if (!companies || companies.length === 0) {
        console.log('No Workwise companies found.');
        return;
    }
