...
// ─── Join Company Processing ────────────────────────────────────────────
async function processJoinCompany(company) {
    ...
    // ─── LAYER 1: Try Join API ──────────────────────────────────────────
    // Try to extract company slug from URL first
    let slug = null;
    if (careerUrl && careerUrl.includes('join.com')) {
        let match = careerUrl.match(/https?:\/\/([^.]+)\.join\.com/);
        if (match && match[1] !== 'www') {
            slug = match[1];
        } else {
            match = careerUrl.match(/join\.com\/companies\/([^\/?]+)/);
            if (match) {
                slug = match[1];
            }
        }
    ...
    // If no slug, guess from company name (clean)
    if (!slug) {
        slug = company.Name
            .toLowerCase()
            .replace(/[^a-z0-9]/g, '')
            ...
            .trim();
        if (slug.length > 2) {
            console.log(`   ⚠️ Using guessed slug: ${slug}`);
        } else {
            console.log(`   ❌ Could not find Join slug for ${company.Name}`);
    ...
    // Try API endpoints
    const apiEndpoints = [
        `https://api.join.com/v1/companies/${slug}/jobs`,
        `https://jobs.join.com/api/v1/companies/${slug}/positions`,
        `https://join.com/api/v1/companies/${slug}/jobs`,
        `https://${slug}.join.com/api/jobs`
    ];
        ...
        try {
            ...
            for (const item of items) {
                ...
                jobs.push({
                    ...
                    apply_url: item.url || `https://join.com/jobs/${item.id}`,
                    ats_source: 'join'
                });
            }
            if (jobs.length > 0) {
    ...
    // ─── LAYER 2: HTML scraping of career page ──────────────────────────
    if (jobs.length === 0) {
        ...
        try {
            ...
            if (scraped > 0) {
                console.log(`   ✅ Layer 2: Found ${scraped} jobs via HTML scraping`);
            }
        } catch (err) {
...
async function run() {
    ...
    if (!companies || companies.length === 0) {
        console.log('No Join companies found.');
        return;
    }
