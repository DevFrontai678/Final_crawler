...
// ─── PROCESS ONE COMPANY ──────────────────────────────────────────────
async function processSuccessFactorsCompany(company) {
    console.log(`\n🔍 Processing: ${company.Name}`);
    console.log(`   URL: ${company.detected_career_url}`);

    const sfInfo = detectSuccessFactorsUrl(company);
    if (!sfInfo) {
        console.log(`   ❌ Could not detect SuccessFactors URL for ${company.Name}`);
        return { company, jobs: [], error: 'No URL detected' };
    }

    const { baseUrl, slug } = sfInfo;
    ...
    // ─── LAYER 2: HTML Scraping on the SuccessFactors base URL ──────
    if (jobs.length === 0) {
...
async function run() {
    ...
    if (!companies || companies.length === 0) {
        console.log('No SuccessFactors companies found.');
        return;
    }

    console.log(`📋 Processing ${companies.length} SuccessFactors companies in batches of 10...\n`);
    ...
    console.log(`\n✅ Done! Total SuccessFactors jobs saved: ${totalJobs}`);
}
